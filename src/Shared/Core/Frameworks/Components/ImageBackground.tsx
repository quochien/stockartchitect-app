...
const isValidSource = (source: number | { uri: any }) =>
 ...
 */
let externalPropsTransformer;
type Props = React.ComponentProps<typeof RNImageBackground> & {
  theme: Theme;
  style?: any;
  styleName?: string;
};
class ImageBackground extends PureComponent<Props, { transformedProps: any }> {
  nativeComponent: any;
  /**
  ...
  render() {
    ...
    return <RNImageBackground {...transformedProps} />;
  }
}
const wrappedWithTheme = withTheme(ImageBackground);

const wrappedStyle = connectStyle("shoutem.ui.ImageBackground")(
  connectAnimation(wrappedWithTheme)
);
export { wrappedStyle as ImageBackground };
